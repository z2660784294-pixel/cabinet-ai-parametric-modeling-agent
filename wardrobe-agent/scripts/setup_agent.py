#!/usr/bin/env python3
"""
Create directory links so Cursor (workspace/.cursor/) or Claude Code
(workspace/.claude/) shares workspace/agents/ and workspace/skills/ on macOS,
Linux, and Windows.
...
# (link_path, target_path) — target is canonical content under workspace/
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "workspace/agents/ and workspace/skills/."
        )
    )
    parser.add_argument(
        "tool",
        choices=("cursor", "claude"),
        help="Which tool to wire: cursor (workspace/.cursor/) or claude (workspace/.claude/).",
    )
...
def main() -> int:
    ...
    print(f"done — {label} will read shared agents/ and skills/ under workspace/.")
    return 0
